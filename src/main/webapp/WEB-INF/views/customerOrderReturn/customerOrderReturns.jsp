...
<div ng-controller="appCTRL">
	...
	<div>
		<form class="form-inline"
			action="<c:url value="/customerOrderReturns"/>">
			...
			<div class="form-group">
				<label for="from"><spring:message
						code="customerOrderReturns.from" /></label> <input
					class="form-control mx-sm-3" id="from" name="from" readonly
         ...
         value = "${from}" />" />
			...
			<div class="form-group">
				<label for="from"><spring:message
						code="customerOrderReturns.to" /></label> <input
					class="form-control mx-sm-3" id="to" name="to" readonly
					value="<fmt:formatDate pattern = "yyyy-MM-dd"
         value = "${to}" />" />
			</div>
			...
			<tbody>
				...
				<c:forEach items="${customerOrderReturns}" var="item">
					<tr>
						...
						<td><fmt:formatNumber value="${item.totalpaid}" /> <c:set
								var="sumTotalPaid" value="${sumTotalPaid+item.totalpaid}" /></td>
						<td>
