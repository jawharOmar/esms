...
<script type="text/javascript">
	...
	var url = "<c:url value="/attachedFiles/1/" />";
</script>
<div ng-app="app" ng-controller="appCTRL" ng-init="init()">
	<h2>
		<spring:message code="editOrderPreProduct.title" />
	</h2>
	<hr>
	...
	<div id="freeze">
			...
			<table id="detail-table" class="table table-bordered">
				...
				<tbody>
					<tr ng-repeat="item in orderPreProduct.preProducts">
						<td>{{$index+1}}</td>
						<td><show-image file="files[$index]"> </show-image> <img
							ng-if="item.attachedFile" class="cus-avatar"
							src="{{url+item.attachedFile.id}}"/></td>
						<td>{{item.code}}</td>
						<td>{{item.name}}</td>
						<td>{{item.weight}}</td>
		...
		<div class="border-top pt-2" ng-form name="form">
			...
			<button ng-disabled="!orderPreProduct.preProducts.length>0"
				class="btn btn-warning" ng-click="saveOrderPreProduct()">
