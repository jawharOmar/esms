...
app.controller('appCTRL', function ($scope, $http, $q) {
    ...
    $scope.init = function () {
		...
		$("#customer-autoselect").autocomplete({
			...
			select: function (event, ui) {
				...
				$scope.customerOrderReturn.customer = item;
				$scope.$digest();
			}
		});
    ...
    $scope.totalPrice = function () {
        ...
        for (var i = 0; i < $scope.customerOrderReturn.productStepUps.length; i++) {
            totalPrice += $scope.customerOrderReturn.productStepUps[i].paymentAmount;
        }
        return totalPrice;
    }
    $scope.setTotalPayment = function () {
        return $scope.customerOrderReturn.totalpaid = parseFloat(($scope
        ...
        $http
            ...
            .then(
                function (response) {
                    ...
                    $scope.productStepUp.returnPrice = $scope.productStepUp.paymentAmount;
    ...
    $scope.editProductStepUp = function (index) {
        ...
        $scope.productStepUp.returnPrice = +($scope.productStepUp.paymentAmount / $scope.productStepUp.quantity)
            .toFixed(3);
    ...
    $scope.saveCustomerOrderReturn = function () {
        ...
        $http({
            ...
            url: $$ContextURL + '/customerOrderReturns/add'
