		...
		(
				...
				function() {
					...
					function convert(jsonObject, options) {
						...
						var mergedOptions = mergeObjects(defaultOptions,
								options || {});

						return convertRecursively(jsonObject, mergedOptions);
					}

					function convertRecursively(jsonObject, options, parentKey,
							carryFormData) {
								...
								if (jsonObject[key] !== null
									...
									if (jsonObject[key] instanceof File) {
										...
										for (var j = 0; j < jsonObject[key].length; j++) {
											...
											formData.append(propName + '[' + j
													+ ']', jsonObject[key]
													.item(j));
										}

									} else if (isArray(jsonObject[key])
											|| isObject(jsonObject[key])) {

										convertRecursively(jsonObject[key],
												options, propName, formData);
