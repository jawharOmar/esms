...
public class ReportDAO {
	...
	public List<DashboardExpiredProductD> getExpiredProducts() {
		...
		Query query = em.createNativeQuery(
				"SELECT I_PRODUCT,PRODUCT_CODE,PRODUCT_NAME,CAST(SUM(QUANTITY-SOLD_QUANTITY) AS SIGNED) AMOUNT \n"
						+ "FROM PRODUCTS INNER JOIN PRODUCT_STEPUPS USING(I_PRODUCT) \n"
						+ "WHERE EXPIRATION_DATE-IFNULL(EXPIRE_ALRAM,0)<=CURDATE() AND QUANTITY-SOLD_QUANTITY>0 \n"
						+ "GROUP BY I_PRODUCT");
