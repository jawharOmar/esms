...
public class BoxAccountingDAO {
    ...
    public List<BoxAccountingD> findBalance(Locale locale) {
        Query query = em.createNativeQuery("select vs.FULL_NAME as FULL_NAME,?1 as TYPE,v.I_ORDER_PRODUCT_STEPUP AS ACTION_ID,v.DISCOUNT,'empty' as NOTE,if(v.REFERENCE_INVOICE_ID is null,'empty',v.REFERENCE_INVOICE_ID)as Invoice,v.ORDER_TIME as Time,0 as Income, if(v.TOTAL_PAYMENT is null,0,v.TOTAL_PAYMENT) as Expense,1 AS TYPE_ID from ORDER_PRODUCT_STEPUPS v INNER JOIN VENDORS vs ON v.I_VENDOR=vs.I_VENDOR where (if(v.TOTAL_PAYMENT is null,0,v.TOTAL_PAYMENT)>0 or v.DISCOUNT>0 )\n" +
                ...
                "UNION ALL\n" +
                "select vs.FULL_NAME as FULL_NAME,?3 as TYPE,vr.I_RETURN AS ACTION_ID,0 AS DISCOUNT,'empty' as NOTE,vr.I_RETURN as Invoice,vr.Time as Time,vr.AMOUNT as Income,0 as Expense,3 AS TYPE_ID from VENDERRETURNS vr INNER JOIN VENDORS vs ON vr.I_VENDOR=vs.I_VENDOR\n" +
                "UNION ALL\n" +
    ...
    public List<BoxAccountingD> findBalanceByDate(Date from, Date to, Locale locale) {
        Query query = em.createNativeQuery("select * from(select vs.FULL_NAME as FULL_NAME,?3 as TYPE,v.I_ORDER_PRODUCT_STEPUP AS ACTION_ID,v.DISCOUNT,'empty' as NOTE,if(v.REFERENCE_INVOICE_ID is null,'empty',v.REFERENCE_INVOICE_ID)as Invoice,v.ORDER_TIME as Time,0 as Income, if(v.TOTAL_PAYMENT is null,0,v.TOTAL_PAYMENT) as Expense,1 AS TYPE_ID  from ORDER_PRODUCT_STEPUPS v INNER JOIN VENDORS vs ON v.I_VENDOR=vs.I_VENDOR where (if(v.TOTAL_PAYMENT is null,0,v.TOTAL_PAYMENT)>0 or v.DISCOUNT>0 )\n" +
                "UNION ALL\n" +
                ...
                "UNION ALL\n" +
                "select vs.FULL_NAME as FULL_NAME,?5 as TYPE,vr.I_RETURN AS ACTION_ID,0 AS DISCOUNT,'empty' as NOTE,vr.I_RETURN as Invoice,vr.Time as Time,vr.AMOUNT as Income,0 as Expense,3 AS TYPE_ID  from VENDERRETURNS vr INNER JOIN VENDORS vs ON vr.I_VENDOR=vs.I_VENDOR\n" +
                "UNION ALL\n" +
    ...
    public HashMap<String, Double> findIncomesAndExpensesByDate(Date date) {
        Query query = em.createNativeQuery("select ROUND(SUM(Income),3) As Incomes, ROUND(SUM(Expense),3) AS Expenses from(select v.ORDER_TIME as Time,0 as Income, if(v.TOTAL_PAYMENT is null,0,v.TOTAL_PAYMENT) as Expense from ORDER_PRODUCT_STEPUPS v INNER JOIN VENDORS vs ON v.I_VENDOR=vs.I_VENDOR where (if(v.TOTAL_PAYMENT is null,0,v.TOTAL_PAYMENT)>0 or v.DISCOUNT>0 )\n" +
                "UNION ALL\n" +
                ...
                "UNION ALL\n" +
                "select vr.Time as Time,vr.AMOUNT as Income,0 as Expense from VENDERRETURNS vr INNER JOIN VENDORS vs ON vr.I_VENDOR=vs.I_VENDOR\n" +
                "UNION ALL\n" +
    ...
    public List<BoxAccountingD> findLoan(Locale locale) {
        ...
        Query loan = em.createNativeQuery("SELECT ?1 as FULL_NAME,?3 as Type, 0 AS ACTION_ID, ROUND(IFNULL((SELECT SUM(IFNULL((DISCOUNT),0)) AS DISCOUNT FROM CUSTOMER_ORDERS),0)+ IFNULL((SELECT SUM(IFNULL((DISCOUNT),0)) AS DISCOUNT FROM CUSTOMER_PAYMENTS),0),3) AS DISCOUNT,'empty' as NOTE,'empty' as Invoice,NOW() as Time,ROUND(IFNULL((SELECT SUM(IFNULL(TOTAL_PRICE,0)-IFNULL(DISCOUNT,0)-IFNULL(TOTAL_PAYMENT,0)) FROM CUSTOMER_ORDERS),0)-IFNULL((SELECT SUM(IFNULL(TOTAL_PRICE-TOTAL_PAID,0)) FROM CUSTOMER_ORDER_RETURNS ),0)-IFNULL((SELECT SUM(TOTAL_PAYMENT) + SUM(if(DISCOUNT is null,0,DISCOUNT)) FROM CUSTOMER_PAYMENTS),0),3) as Income,0  as Expense,10 AS TYPE_ID \n" +
                "UNION ALL\n" +
                ...
                " FROM VENDOR_PAYMENTS vp ),0)-IFNULL((SELECT SUM(AMOUNT) FROM VENDERRETURNS ),0) ,3) as Expense,10 AS TYPE_ID ");
    ...
    public List<BoxAccountingD> findLoanByDate(Date to, Locale locale) {
        ...
        Query loan = em.createNativeQuery("  SELECT ?2 as FULL_NAME, ?4 as Type, 0 AS ACTION_ID,ROUND(IFNULL((SELECT SUM(IFNULL((DISCOUNT),0)) AS DISCOUNT FROM CUSTOMER_ORDERS WHERE ORDER_TIME < ?1),0)+ IFNULL((SELECT SUM(IFNULL((DISCOUNT),0)) AS DISCOUNT FROM CUSTOMER_PAYMENTS WHERE PAYMENT_TIME < ?1),0),3) AS DISCOUNT ,'empty' as NOTE,'empty' as Invoice,str_to_date(concat(?1,' ',TIME_FORMAT(current_time(),'%T')),'%Y-%m-%d %h:%i:%s') as Time,ROUND(IFNULL((SELECT SUM(IFNULL(TOTAL_PRICE,0)-IFNULL(DISCOUNT,0)-IFNULL(TOTAL_PAYMENT,0)) FROM CUSTOMER_ORDERS WHERE ORDER_TIME  < ?1),0)-IFNULL((SELECT SUM(IFNULL(TOTAL_PRICE-TOTAL_PAID,0)) FROM CUSTOMER_ORDER_RETURNS WHERE CUSTOMER_ORDER_RETURN_TIME  < ?1),0)-IFNULL((SELECT SUM(TOTAL_PAYMENT) + SUM(if(DISCOUNT is null,0,DISCOUNT)) FROM CUSTOMER_PAYMENTS WHERE PAYMENT_TIME <?1),0),3) as Income, 0 as Expense,10 AS TYPE_ID \n" +
                ...
                "-IFNULL((SELECT SUM(AMOUNT) FROM VENDERRETURNS WHERE Time  < ?1),0) ,3) as Expense,10 AS TYPE_ID");
    ...
    public List<BoxAccountingD> findCustomerPayment(Date from, Date to, Locale locale) {
        ...
        Query query = em.createNativeQuery("select c.FULL_NAME as FULL_NAME,?3 as TYPE,p.I_CUSTOMER_PAYMENT as ACTION_ID ,IF(DISCOUNT IS NULL,0,DISCOUNT) AS DISCOUNT,if(p.NOTE is null,'empty',p.NOTE) as NOTE,p.I_CUSTOMER_PAYMENT as Invoice,p.PAYMENT_TIME as Time,p.TOTAL_PAYMENT as Income,0 as Expense,5 AS TYPE_ID from CUSTOMER_PAYMENTS p INNER JOIN CUSTOMERS c ON p.I_CUSTOMER=c.I_CUSTOMER where p.PAYMENT_TIME between ?1 and ?2 order by p.PAYMENT_TIME;");
        query.setParameter(1, from);
        query.setParameter(2, to);
    ...
    public HashMap<String, Double> findCustomerPaymentIncomesByDate(Date date) {
        ...
        Query query = em.createNativeQuery("select ROUND(SUM(TOTAL_PAYMENT),3) as Income from CUSTOMER_PAYMENTS where PAYMENT_TIME < ?1");
        query.setParameter(1, date);
    ...
    public List<BoxAccountingD> findCustomerOrder(Date from, Date to, Locale locale) {
        Query query = em.createNativeQuery("select c.FULL_NAME as FULL_NAME,?3 as TYPE,o.I_CUSTOMER_ORDER AS ACTION_ID,IF(DISCOUNT IS NULL,0,DISCOUNT) AS DISCOUNT,if(o.NOTE is null,'empty',o.NOTE) as NOTE,o.INVOICE_ID as Invoice,o.ORDER_TIME as Time,if(o.TOTAL_PAYMENT is null,0,o.TOTAL_PAYMENT) as Income, (o.TOTAL_PRICE-if(o.DISCOUNT is null,0,o.DISCOUNT)) as Expense,4 AS TYPE_ID from CUSTOMER_ORDERS o INNER JOIN CUSTOMERS c ON o.I_CUSTOMER=c.I_CUSTOMER where (if(o.TOTAL_PAYMENT is null,0,o.TOTAL_PAYMENT)>0 or o.DISCOUNT>0) and o.ORDER_TIME between ?1 and ?2 order by o.ORDER_TIME;");
        query.setParameter(1, from);
        query.setParameter(2, to);
    ...
    public HashMap<String, Double> findCustomerOrderIncomesByDate(Date date) {
        ...
        Query query = em.createNativeQuery("select ROUND(SUM(if(TOTAL_PAYMENT is null,0,TOTAL_PAYMENT)),3) as Income from CUSTOMER_ORDERS where ORDER_TIME<?1");
        query.setParameter(1, date);
    ...
    public HashMap<String, Double> findCustomerReturnIncomesByDate(Date date) {
        ...
        Query query = em.createNativeQuery("select ROUND(SUM(TOTAL_PAID),3) as Expense from CUSTOMER_ORDER_RETURNS  where CUSTOMER_ORDER_RETURN_TIME < ?1");
        query.setParameter(1, date);
    ...
    public List<BoxAccountingD> findVendorPayment(Date from, Date to, Locale locale) {
        Query query = em.createNativeQuery("select vs.FULL_NAME as FULL_NAME,?3 as TYPE,vp.I_VENDOR_PAYMENT AS ACTION_ID,IF(vp.DISCOUNT IS NULL,0,vp.DISCOUNT) AS DISCOUNT,if(vp.NOTE is null,'empty',vp.NOTE) as NOTE,vp.I_VENDOR_PAYMENT as Invoice,vp.PAYMENT_TIME as Time,0 as Income,vp.TOTAL_PAYMENT as Expense,2 AS TYPE_ID from VENDOR_PAYMENTS vp INNER JOIN VENDORS vs ON vp.I_VENDOR=vs.I_VENDOR where vp.PAYMENT_TIME between ?1 and ?2 order by vp.PAYMENT_TIME;");
        query.setParameter(1, from);
        query.setParameter(2, to);
    ...
    public HashMap<String, Double> findVendorPaymentExpenseByDate(Date date) {
        ...
        Query query = em.createNativeQuery("select ROUND(SUM(TOTAL_PAYMENT),3) as Expense from VENDOR_PAYMENTS where PAYMENT_TIME < ?1");
        query.setParameter(1, date);
    ...
    public List<BoxAccountingD> findVendorOrder(Date from, Date to, Locale locale) {
        Query query = em.createNativeQuery("select vs.FULL_NAME as FULL_NAME,?3 as TYPE,v.I_ORDER_PRODUCT_STEPUP AS ACTION_ID,v.DISCOUNT,'empty' as NOTE,if(v.REFERENCE_INVOICE_ID is null,'empty',v.REFERENCE_INVOICE_ID)as Invoice,v.ORDER_TIME as Time,0 as Income, if(v.TOTAL_PAYMENT is null,0,v.TOTAL_PAYMENT) as Expense,1 AS TYPE_ID from ORDER_PRODUCT_STEPUPS v INNER JOIN VENDORS vs ON v.I_VENDOR=vs.I_VENDOR where (if(v.TOTAL_PAYMENT is null,0,v.TOTAL_PAYMENT)>0 or v.DISCOUNT>0 ) and  v.ORDER_TIME between ?1 and ?2 order by v.ORDER_TIME;");
        query.setParameter(1, from);
        query.setParameter(2, to);
        query.setParameter(3, messageSource.getMessage("boxAccounting.vendorOrders", null, locale));
    ...
    public HashMap<String, Double> findVendorOrderExpenseByDate(Date date) {
        ...
        Query query = em.createNativeQuery("select ROUND(SUM(if(TOTAL_PAYMENT is null,0,TOTAL_PAYMENT)),3)  as Expense from ORDER_PRODUCT_STEPUPS where ORDER_TIME < ?1");
        query.setParameter(1, date);
    ...
    public List<BoxAccountingD> findVendorReturn(Date from, Date to, Locale locale) {
        Query query = em.createNativeQuery("select vs.FULL_NAME as FULL_NAME,?3 as TYPE,vr.I_RETURN AS ACTION_ID,0 AS DISCOUNT,'empty' as NOTE,vr.I_RETURN as Invoice,vr.Time as Time,vr.AMOUNT as Income,0 as Expense,3 AS TYPE_ID from VENDERRETURNS vr INNER JOIN VENDORS vs ON vr.I_VENDOR=vs.I_VENDOR where vr.Time between ?1 and ?2 order by vr.Time;");
        query.setParameter(1, from);
        query.setParameter(2, to);
        query.setParameter(3, messageSource.getMessage("boxAccounting.vendorReturn", null, locale));
    ...
    public HashMap<String, Double> findVendorReturnExpenseByDate(Date date) {
        ...
        Query query = em.createNativeQuery("select ROUND(SUM(AMOUNT),3) as Income from VENDERRETURNS vr where vr.Time < ?1");
        query.setParameter(1, date);
    ...
    public HashMap<String, Double> findOldExpenses(Date date) {
        ...
        Query query = em.createNativeQuery("select ROUND(SUM(AMOUNT),3) as Expense from EXPENSES where EXPENSE_TIME <?1");
        query.setParameter(1, date);
    ...
    public HashMap<String, Double> findOldWithdraws(Date date) {
        ...
        Query query = em.createNativeQuery("SELECT ROUND(SUM(AMOUNT),3) AS Expense FROM WITHDRAW WHERE WITHDRAW_TIME <?1");
        query.setParameter(1, date);
