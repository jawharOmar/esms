...
//	Index For Excel File
    ...
    @Override
    public ByteArrayInputStream stockToExcel(List<ProductD> productDs, Locale local, boolean cost_role, List<StockColumnD> selectedColumn) throws IOException {
        String[] columnsFixed = {"stock.code", "stock.name", "stock.category", "stock.unitType", "stock.stockLevel",
                "stock.stockPacketLevel", "stock.lastPrice", "stock.cost", "stock.totalCost"
                , "stock.packetSize", "stock.minimumStockLevel", "stock.price"};

        //      Export Column if True
        boolean code, name, category, unitType, stockLevel, stockPacketLevel, lastPrice,
                cost, totalCost, packetSize, minimumStockLevel, price;
        code = name = category = unitType = stockLevel = stockPacketLevel = lastPrice =
                cost = totalCost = packetSize = minimumStockLevel = price = true;
        ...
        for (String columnName : columnsFixed) {
            ...
            for (StockColumnD tableColumn : selectedColumn) {
                ...
                if (code && columnName.equals(columnsFixed[0]) && cname.equals(tableColumn.getName())) {
                    ...
                    stockLevel = false;
                    exist = true;
                } else if (stockPacketLevel && columnName.equals(columnsFixed[5]) && cname.equals(tableColumn.getName())) {
                    ...
                    packetSize = false;
                    exist = true;
                } else if (minimumStockLevel && columnName.equals(columnsFixed[10]) && cname.equals(tableColumn.getName())) {
...
//            Default Value (1)
            ...
            for (ProductD productD : productDs) {
                ...
                if (stockLevel) {
                    row.createCell(index()).setCellValue((round(productD.getStockLevel(), 3) + " " + (productD.getUnit() != null ? productD.getUnit() : "")));
                }
                if (stockPacketLevel) {
                    row.createCell(index()).setCellValue((productD.getPacketSize() != null ? round((productD.getStockLevel() / productD.getPacketSize()), 3) : 0));
                }

                if (cost_role) {
                    if (lastPrice) {
                        row.createCell(index()).setCellValue((productD.getLastPrice() != null ? round(productD.getLastPrice(), 3) : 0));
                    }

                    if (cost) {
                        row.createCell(index()).setCellValue((productD.getCost() != null ? round(productD.getCost(), 3) : 0));
                    }

                    if (totalCost) {
                        int index = index();
                        totalCostIndex = index;
                        sumTotalCost += productD.getCost() != null ? round((productD.getStockLevel() * productD.getCost()), 3) : 0;
                        row.createCell(index).setCellValue((productD.getCost() != null ? round((productD.getStockLevel() * productD.getCost()), 3) : 0));
                    }
                }

                if (packetSize) {
                    row.createCell(index()).setCellValue((productD.getPacketSize() != null ? productD.getPacketSize() : 0));
                }
                if (minimumStockLevel) {
                    row.createCell(index()).setCellValue((productD.getMinimumStockLevel() != null ? productD.getMinimumStockLevel() : 0));
                }
                if (price) {
                    row.createCell(index()).setCellValue((productD.getPrice() != null ? round(productD.getPrice(), 3) : 0));
                }
                ...
                for (PriceCategory priceCategory : priceCategories) {
                    ...
                    if (!exist) {
                        if (priceCategory.getRatio() != null) {
                            row.createCell(index()).setCellValue((productD.getCost() != null ? round(productD.getCost() + productD.getCost() * priceCategory.getRatio().doubleValue(), 3) : 0));
                        } else {
            ...
            if (cost_role) {
                ...
                row.createCell(totalCostIndex).setCellValue(
                        new DecimalFormat(".###").format(
                                round(sumTotalCost, 3)));
            }
    ...
    public ProductD findByProductIdAndStockId(Integer productId, Integer stockId) {
        ...
        List<ProductPriceCategory> productPriceCategories = productPriceCategoryService.findAllByPorductId(productD.getProductId());
